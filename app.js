		...
		return $author$project$PhotoFolders$Folder(
			{
				expanded: true,
				name: name,
				photoUrls: $elm$core$Dict$keys(photos),
...
var $author$project$PhotoFolders$finishPhoto = function (_v0) {
	...
	return _Utils_Tuple2(
		url,
		{relatedUrls: json.relatedUrls, size: json.size, title: json.title, url: url});
};
...
var $author$project$PhotoGallery$buildPhoto = F3(
...
var $author$project$PhotoGallery$init = function (flags) {
	var activity = function () {
		...
		if (_v0.$ === 'Ok') {
			var version = _v0.a;
			return 'Initializing Pasta v' + $elm$core$String$fromFloat(version);
		} else {
			return 'No pasta version number';
		}
	}();
	return _Utils_Tuple2(
		_Utils_update(
...
var $author$project$Main$parser = $elm$url$Url$Parser$oneOf(
	_List_fromArray(
		[
			A2($elm$url$Url$Parser$map, $author$project$Main$Folders, $elm$url$Url$Parser$top),
			A2(
...
var $author$project$PhotoGallery$activityChanges = _Platform_incomingPort('activityChanges', $elm$json$Json$Decode$value);
var $author$project$PhotoGallery$subscriptions = function (_v0) {
...
var $author$project$PhotoGallery$applyFilters = function (model) {
	...
	switch (_v0.$) {
		case 'Loaded':
			...
			var url = $author$project$UrlConstants$urlPrefix + ('large/' + selectedUrl);
	...
	function (msg, model) {
		switch (msg.$) {
			...
			case 'GotPhotos':
				if (msg.a.$ === 'Ok') {
					var photos = msg.a.a;
					var _v3 = $elm$core$List$head(photos);
					if (_v3.$ === 'Just') {
						var photo = _v3.a;
						return $author$project$PhotoGallery$applyFilters(
							_Utils_update(
								model,
								{
									status: A2($author$project$PhotoGallery$Loaded, photos, photo.url)
								}));
					} else {
						return _Utils_Tuple2(
							_Utils_update(
			...
			default:
				var value = msg.a;
				var _v4 = A2($elm$json$Json$Decode$decodeValue, $elm$json$Json$Decode$string, value);
				if (_v4.$ === 'Ok') {
					var status = _v4.a;
					return _Utils_Tuple2(
						_Utils_update(
...
var $author$project$Main$browserTitle = function (page) {
	switch (page.$) {
		case 'FoldersPage':
			return 'Photo Pholders';
		case 'GalleryPage':
			return 'Photo Gallery';
		default:
			return 'Photo Groove, SPA Style';
	}
};
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_Utils_ap($author$project$UrlConstants$urlPrefix, thumb.url)),
...
var $author$project$PhotoGallery$viewLoaded = F4(
	function (photos, selectedUrl, chosenSize, model) {
		return _List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						...
						$author$project$PhotoGallery$sizeToString(chosenSize))
					]),
				A2(
					$elm$core$List$map,
		...
		function () {
			...
			switch (_v0.$) {
				case 'Loaded':
					var photos = _v0.a;
					var selectedUrl = _v0.b;
					return A4($author$project$PhotoGallery$viewLoaded, photos, selectedUrl, model.chosenSize, model);
				case 'Loading':
					return _List_Nil;
				default:
...
var $author$project$Main$view = function (model) {
	...
	return {
		...
		title: $author$project$Main$browserTitle(model.page)
	};
};
